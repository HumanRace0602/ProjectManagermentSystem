...
namespace SystemWebUI.UserPage.ProjectDocuments
{
    public partial class Plan : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Session["userName"].ToString() != "traveller")
            {
                if (UserProject.IsAllowReadProject(Int32.Parse(Request.QueryString["id"]), Session["userName"].ToString()))
                {
                    ...
                    string path = Server.MapPath("~/App_Data/Projects") + "\\" + Request.QueryString["id"] + "\\Plan.txt";
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Response.Redirect("FreeTextBox.aspx?id=" + Request.QueryString["id"] + "&projectName=" + Request.QueryString["projectName"] + "&documentType=plan");
            }
